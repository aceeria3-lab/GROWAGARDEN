...
createCustomButton("Section 3", "Auto Cook Ham Stew", "Automatically Cook Ham Stew Sequence", function()
    task.spawn(function()
        triggerPrompt("Pot")
        interactWithCounter()
...
createCustomButton("Section 3", "Auto Cook Chicken Soup", "Automatically Cook Chicken Soup Sequence", function()
    task.spawn(function()
        triggerPrompt("Pot")
        interactWithCounter()
...
createCustomButton("Section 3", "Auto Prepare Eyeball & Spaghetti", "Automatically Cook Eyeball and Spaghetti Sequence", function()
    task.spawn(function()
        triggerPrompt("Bowl")
        interactWithCounter()
...
createCustomButton("Section 3", "End Chase 2", "Teleport to Trigger End", function()
    ...
    local rootPart = character and character:FindFirstChild("HumanoidRootPart")
    if rootPart then
        rootPart.CFrame = CFrame.new(-3341.951, 1237.817, -6793.478)
    end
end)
...
-- ====================================================================
createCustomButton("Section 5", "Auto Find Daughter", "Automatically Find Daughter and Back to SafeZone", function()
    local player = game:GetService("Players").LocalPlayer
    local character = player.Character
    local rootPart = character and character:FindFirstChild("HumanoidRootPart")
...
-- ====================================================================
-- Mother ESP Only (Safe Ashina Format - No SafeZone)
-- ====================================================================
local motherConnection = nil
local motherTracker = {}

createToggle("Section 5", "Mother ESP", "Show Highlights on Mother", function(state)
    local player = game:GetService("Players").LocalPlayer
...
-- ====================================================================
createCustomButton("Section 5", "Find Door Exit", "Automatically Find the Exit Door", function()
    local player = game:GetService("Players").LocalPlayer
    local workspace = game:GetService("Workspace")
...
-- ====================================================================
createCustomButton("Section 5", "Auto Collect Notes", "Automatically Found and Collect Notes", function()
    local player = game:GetService("Players").LocalPlayer
    local workspace = game:GetService("Workspace")
...
-- ====================================================================
-- CreateToggle: Nagisa ESP (HITBOX & HEADHITBOX Only + Transparency Fix)
-- ====================================================================
local espEnabled = false
local espData = {} -- Dito natin ise-save ang original transparency at highlights

createToggle("Section 5", "Nagisa ESP", "Add Highlights on Nagisa Hitboxes", function(state)
    espEnabled = state
            ...
            while espEnabled do
                for _, desc in ipairs(workspace:GetDescendants()) do
                    if desc:IsA("BasePart") then
                        ...
                        -- Saktong pangalan na may tamang capitalization
                        if name == "HITBOX" or name == "HEADHITBOX" then
                            if not espData[desc] then
                                -- I-save ang original transparency bago baguhin
                                local origTrans = desc.Transparency
                                
                                -- Gawing visible (0.3) para lumitaw ang Highlight kung naka-invisible man
                                ...
                                espData[desc] = {Highlight = hl, OriginalTrans = origTrans}
                            end
                        end
                    end
                end
                task.wait(1.5) -- Refresh every 1.5 seconds para sa mga bagong spawn/load
            end
        end)
    else
        -- Alisin ang highlights at ibalik sa original transparency kapag naka-off
        for part, data in pairs(espData) do
            if part and part.Parent then
                pcall(function()
                    part.Transparency = data.OriginalTrans
                end)
            end
...
createToggle("Section 5", "Auto SafeSpot Poison Laser", "You can still Get Damage if you staying on the Cannons", function(state)
...
-- ====================================================================
createCustomButton("Section 5", "Auto Collect Cannon Ball", "Automatically Collect 5 Cannon Ball", function()
    local player = game:GetService("Players").LocalPlayer
    local workspace = game:GetService("Workspace")
...
-- ====================================================================
createCustomButton("Section 5", "Get Cutlass", "Automatically Collect Cutlass Sword", function()
    local player = game:GetService("Players").LocalPlayer
    local workspace = game:GetService("Workspace")
...
-- ====================================================================
createCustomButton("Section 5", "Auto Slash Nagisa Serpent", "You Need Timing Cuz it's Very Risky to Lock on Boss", function(btnObject)
    local player = game:GetService("Players").LocalPlayer
    local workspace = game:GetService("Workspace")
